...
class MyFrameQ10 extends JFrame implements ActionListener{
	...
	public     static boolean containsAzero(ArrayList list, int target){              // to be completed
        if (list.size()==0) return false;
	   else{     int first= (Integer)list.get(0);
	                list.remove(0);
	                if (first==target) return true;
                   else          return  containsAzero(list,target);
	...
	public void actionPerformed(ActionEvent e){
 	...
 	if (target==b2){ 
  		boolean res = containsAzero(new ArrayList(list), 0);
  		t3.setText(""+res);
     	}
     }
	 }
