...
class MyFrameRecursionQ2 extends JFrame implements ActionListener{
	...
	private JButton b1=new JButton("SumDoubleDigit");private JTextField t2=new JTextField("0",15);
	private JButton b2=new JButton("CountZeros");private JTextField t3=new JTextField("0",15);
	...
	public     static int sumDoubleDigit(ArrayList li){              // to be completed
		if(li.size() == 0) return 0;
		int first = (int) li.get(0);
		li.remove(0);
		if(first/10 > 0) {
			return first + sumDoubleDigit(li);
		}else {
			return 0 + sumDoubleDigit(li);
		}
 	   }
	
	public     static int countZeros(ArrayList li){              // to be completed
		if(li.size() == 0) return 0;
		int first = (int) li.get(0);
		li.remove(0);
		if(first == 0) {
			return 1 + countZeros(li);
		}else {
			return 0 + countZeros(li);
		}
 	   }
